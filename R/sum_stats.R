#' Total Annual Discharge Statistics
#'
#' ANOVA or Kruskal-Wallis ranked test of the z-scored sum discharge values.
#'
...
sum_stats <- function(project_directory, normal = FALSE){
  ...
  data_z <- read.csv(paste0(project_directory, "sum_z.csv"))
  ...
  if (normal == FALSE){
    ...
    boxplot(data_stats$ranked~data_stats$enso, col = c("salmon","snow3","skyblue"), main = "Z-scored Sum Ranks", xlab = "ENSO Category", ylab = "Z-scored Discharge Value")
  }
  ...
  if (normal == TRUE){
    ...
    # boxplot of the data
    boxplot(data_stats$value~data_stats$enso, col = c("salmon","snow3","skyblue"), main = "Z-scored Sum Values", xlab = "ENSO Category", ylab = "Z-scored Discharge Value")
  }

...
mean_kw <- function(project_directory){
  ...
  print(dunn_results)
  mean_anova$ranked <- rank(mean_anova$value)
  boxplot(mean_anova$ranked~mean_anova$enso, col = c("salmon","snow3","skyblue"), main = "Z-scored Mean Ranks", xlab = "ENSO Category", ylab = "Z-scored Discharge Value")

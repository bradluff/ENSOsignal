...
min_kw <- function(project_directory){
  ...
  min_nino_anova <- as.data.frame(matrix(nrow = length(min_nino)*length(min_nino$`1953`), ncol = 2))
  ...
  min_neutral_anova <- as.data.frame(matrix(nrow = length(min_neutral)*length(min_neutral$`1951`), ncol = 2))
  ...
  print(dunn_results)
  min_anova$ranked <- rank(min_anova$value)
  boxplot(min_anova$ranked~min_anova$enso, col = c("salmon","snow3","skyblue"), main = "Z-scored Minimum Ranks", xlab = "ENSO Category", ylab = "Z-scored Discharge Value")
